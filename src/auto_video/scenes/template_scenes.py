...
class IntroScene(BaseProductionScene):
    ...
    def play_on(self, scene):
        ...
        scene.play(FadeIn(accent, scale=0.95), FadeIn(content, shift=UP * 0.3), run_time=0.8)
...
class DynamicContentScene(BaseProductionScene):
    ...
    def play_on(self, scene):
        icon = Text(VISUAL_ICON_MAP.get(self.visual, "\u25c6"), font=THEME["font"], font_size=64, color=self.accent_color)
        icon_bg = Circle(radius=0.6, color=self.accent_color, stroke_width=2, fill_opacity=0.1).move_to(icon)
        ...
        header.set_max_width(scene.camera.frame_width * 0.9) # Safety margin
        ...
        scene.play(
            FadeIn(icon_group, scale=1.2, shift=UP*0.5),
            Write(headline),
            run_time=0.5
        )
    ...
    def _build_bullets(self, scene, header):
        ...
        for index, bullet in enumerate(texts[:3]):
            ...
            line.set_max_width(scene.camera.frame_width * 0.7)
            row = VGroup(marker, line).arrange(RIGHT, buff=0.3, aligned_edge=UP)
            group.add(row)
        
        group.arrange(DOWN, buff=0.8, aligned_edge=LEFT)
        group.set_max_width(scene.camera.frame_width * 0.85) # Safety margin
        group.move_to(ORIGIN)
        return group

    def _build_caption(self, scene):
        chunk_size = 1  # Word-by-word for better sync
        chunks = [self.words_timing[i:i + chunk_size] for i in range(0, len(self.words_timing), chunk_size)]
        caption_words = []
        for chunk in chunks:
            ...
            mob.set_max_width(scene.camera.frame_width * 0.85)
            # Premium shadow effect
            shadow = mob.copy().set_color(BLACK).shift(0.05 * DOWN + 0.05 * RIGHT).set_opacity(0.5)
...
class OutroScene(BaseProductionScene):
    ...
    def play_on(self, scene):
        ...
        ring.set_max_width(scene.camera.frame_width * 0.8) # More conservative safety

        scene.play(FadeIn(cta_text, scale=0.95), Create(ring), run_time=0.7)
        scene.play(ring.animate.scale(1.05).set_stroke(THEME["secondary_color"], 5), rate_func=there_and_back, run_time=0.8)
        scene.wait(1.5)
